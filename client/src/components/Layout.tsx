    ...
    <div className="min-h-screen flex flex-col">
      <nav className="bg-[#1e3a5f] text-white px-2 sm:px-4 py-2 flex items-center justify-between shadow-md gap-2">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          ...
          <Link to="/" className="text-sm sm:text-base hover:text-blue-200 hidden sm:inline">
            Dashboard
          </Link>
          <Link to="/operators" className="text-sm sm:text-base hover:text-blue-200 hidden sm:inline">
            Rufzeichen
          </Link>
          <Link to="/reports" className="text-sm sm:text-base hover:text-blue-200 hidden sm:inline">
            Auswertung
          </Link>
          <Link to="/hilfe" className="text-sm sm:text-base hover:text-blue-200 hidden sm:inline">
            Hilfe
          </Link>
        </div>
        <div className="flex items-center gap-2 sm:gap-4 text-sm flex-shrink-0">
          <span className="text-blue-200 text-sm">{admin?.username}</span>
          <button
            onClick={handleLogout}
            className="bg-red-700 hover:bg-red-800 px-2 sm:px-3 py-1 rounded text-sm"
          >
            Abmelden
          </button>
        </div>
      </nav>
      <div className="sm:hidden bg-[#1e3a5f] text-white px-2 pb-2 flex gap-3 text-sm border-t border-white/10">
      ...
      <main className="p-3 sm:p-5 max-w-[1600px] mx-auto flex-1 w-full">
        <Outlet />
      </main>
      <footer className="bg-gray-100 border-t text-center py-3 text-xs text-gray-500 flex items-center justify-center gap-2 sm:gap-4 flex-wrap px-2">
        ...
        <span>·</span>
        <a href="https://github.com/achildrenmile/bosarsalog/releases/tag/v1.2.4" target="_blank" rel="noopener noreferrer" className="hover:text-gray-700">v1.2.4</a>
      </footer>
    </div>
  );
}
